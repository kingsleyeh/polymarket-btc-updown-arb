...
export interface DashboardStats {
  ...
  bestArb: { market: string; profit: number } | null;
  lastScan: string;
}
 ...
 * Update stats
 */
export function updateStats(update: Partial<DashboardStats>): void {
  stats = { ...stats, ...update };
 ...
 */
export function startDashboardServer(port: number = 3000): express.Application {
  const app = express();
  ...
  app.get('/api/stream', (req: Request, res: Response) => {
    ...
    const onStats = (newStats: DashboardStats) => {
