 ...
 */
export function logArbitrage(entry: ArbitrageLogEntry): void {
  ensureDataDir();
 ...
 */
export function incrementScanCount(marketsScanned: number): void {
  ensureDataDir();
 ...
 * Increment arb count
 */
export function incrementArbCount(): void {
  ensureDataDir();
 ...
 * Get all logged arbs
 */
export function getLoggedArbs(): ArbitrageLogEntry[] {
  try {
    if (fs.existsSync(ARB_LOG_JSON)) {
