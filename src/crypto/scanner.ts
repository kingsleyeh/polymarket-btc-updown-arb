 ...
 */
function is15MinuteMarket(question: string): boolean {
 ...
 */
export async function scanBTCUpDownMarkets(): Promise<UpDownMarket[]> {
  ...
  try {
    ...
    const response = await axios.get(`${GAMMA_API_URL}/events`, {
      ...
      timeout: 5000, // 5s timeout for market discovery
    });
 ...
 * Get market summary
 */
export function getMarketSummary(markets: UpDownMarket[]): {
  total: number;
  ...
  for (const m of markets) {
    ...
    if (timeToExpiry < 600) expiringSoon++; // < 10 minutes
  }
