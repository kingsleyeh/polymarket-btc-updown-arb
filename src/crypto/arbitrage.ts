...
const activeArbs: Map<string, ArbitrageOpportunity> = new Map();

/**
 * Fetch current prices for a market
 * PRIORITY: WebSocket cache (instant) → REST fallback
 */
export async function fetchMarketPrices(market: UpDownMarket): Promise<MarketPrices | null> {
  ...
  try {
    ...
    const upAsks = (upBook.data?.asks || []).sort((a: any, b: any) => parseFloat(a.price) - parseFloat(b.price));
    const downAsks = (downBook.data?.asks || []).sort((a: any, b: any) => parseFloat(a.price) - parseFloat(b.price));
...
export function checkArbitrage(
  ...
  const executableShares = Math.min(
    prices.up_shares_available,
    prices.down_shares_available,
    PAPER_MAX_SHARES
  );
 ...
 */
export function updateArbTracking(
  marketId: string,
  newArb: ArbitrageOpportunity | null,
  ...
  if (existing && !newArb) {
    ...
    let closeReason: 'price_moved' | 'liquidity_exhausted' | 'expiry_cutoff' = 'price_moved';
 ...
 */
export function getActiveArbs(): ArbitrageOpportunity[] {
 ...
 */
export function clearArbs(): void {
  activeArbs.clear();
}
 ...
 */
export function getArbPersistence(arb: ArbitrageOpportunity): number {
